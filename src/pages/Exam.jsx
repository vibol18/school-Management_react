    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-[#f8fafc]">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400">Organize and coordinate assessment checks by track modules and subject lines</p>
        </div>
        <button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl shadow-xs transition">
          + Create New Exam
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {exams.filter(e => 
          ...
          return (
            <div key={exam.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Date Matrix: <span className="font-mono">{exam.examDate}</span></p>
                <p className="text-xs text-slate-500 mt-1">Total Marks: <span className="text-red-600 font-bold">{exam.totalMarks}</span></p>
              </div>
              
              <div className="flex items-center justify-end gap-2 border-t border-slate-50 pt-3 mt-4">
                <button onClick={() => handleEdit(exam)} className="bg-slate-50 hover:bg-amber-500 text-slate-700 hover:text-white px-3 py-1.5 rounded-xl text-xs font-bold transition">Modify</button>
                <button onClick={() => handleDelete(exam.id)} className="bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-600 px-3 py-1.5 rounded-xl text-xs transition">Remove</button>
              </div>
            </div>
          );
        })}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs" value={form.title} required onChange={(e) => setForm({ ...form, title: e.target.value })} />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Select Academic Course Subject</label>
                <select className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs" value={form.subjectId} required onChange={(e) => setForm({ ...form, subjectId: e.target.value })}>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Assign Classroom Target</label>
                <select 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs" value={form.examDate} required onChange={(e) => setForm({ ...form, examDate: e.target.value })} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Max Ceiling Score</label>
                  <input type="number" className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs" value={form.totalMarks} required onChange={(e) => setForm({ ...form, totalMarks: e.target.value })} />
                </div>
              </div>

              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsOpen(false)} className="flex-1 bg-slate-100 py-2 rounded-xl text-xs font-bold">Discard</button>
                <button type="submit" className="flex-1 bg-blue-600 text-white py-2 rounded-xl text-xs font-bold">Save Configuration</button>
              </div>
            </form>
          </div>
        </div>
