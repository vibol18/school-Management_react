...
function Classes() {
  ...
  const fetchData = async () => {
    try {
      ...
      let allExams = [];
      try {
        const examRes = await axios.get("http://localhost:8080/api/exams"); 
      ...
      const updatedClasses = await Promise.all(
        rawData.map(async (c) => { 
          let studentCount = 0;
          try {
            const studentRes = await axios.get(`http://localhost:8080/api/students/class/${c.id}`);
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-[#f8fafc] font-sans">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Manage academic tracks, room allocations, and roster density metrics</p>
        </div>
          ...
          {filtered.map((c) => (
            ...
            >
              ...
              <div className="flex items-center justify-between border-t border-slate-50 pt-3 mt-2">
                <div className="flex items-center gap-1.5 text-slate-500" title="Enrolled Students / Maximum Capacity">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-slate-100">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-left">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Class Name</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Room Number</label>
                  <input
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:bg-white transition"
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Section Shift</label>
                  <input
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:bg-white transition"
